...
impl Planner {
    ...
    fn add_primitive_task_operators(&self, plan:&mut Plan, state:&mut State, body:&Vec<Operation>) -> Result<bool, Error> {
        let mut all = true;
        for op in body {
            all &= match op {
                ...
                Operation::PlanTask(task_id) => self.run_astar(plan, state, *task_id),
    ...
    fn run_astar(&self, plan:&mut Plan, state:&mut State, task_id:usize) -> Result<bool, Error> {
        ...
        match self.domain.tasks.get(task_id).unwrap() {
            Task::Complex(ComplexTask { preconditions, cost, body, effects,.. }) => {
                if let Some((task_plan, _task_plan_cost)) = Astar(Node::new(state, usize::MAX), preconditions, |f| 4, self) {
                    for subtask in task_plan {
                        self.run_astar(plan, state, subtask)?;
                    }

                    let mut method_plans = PriorityQueue::new();
                    for (method_id, method) in body.iter().enumerate() {
                        if let Some((mut method_plan, method_plan_cost)) = Astar(Node::new(state, usize::MAX), &method.preconditions, |f| 4, self) {
                    ...
                    if let Some((mut method_plan, _method_plan_cost)) = method_plans.pop() { // Get the cheapest method to run
                        ...
                        let r = self.add_primitive_task_operators(plan, state, &body[method_id].body);
                        state.eval(effects);
                        r
                    } else {
            ...
            Task::Primitive(PrimitiveTask { preconditions, cost, body, effects }) => {
                if let Some((task_plan, _task_plan_cost)) = Astar(Node::new(state, usize::MAX), preconditions, |f| 4, self) {
                    ...
                    let r = self.add_primitive_task_operators(plan, state, body);
                    state.eval(effects);
                    r
                } else {
                    Ok(false)
    ...
    pub fn plan(&self, state:&State) -> Result<Plan, Error> {
        ...
        self.run_astar(&mut plan, &mut state, self.domain.get_main_id())?;
        Ok(plan)
    }
}