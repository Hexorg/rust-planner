...
pub mod tokens;
use tokens::{Token, Span, TokenKind::*, Literal::*};

pub mod lexer;
use lexer::Lexer;

pub mod expression;
use expression::{Expr, ExprKind};

pub mod statement;
use statement::{Stmt, StmtKind, Block};
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "line:{} col:{} {}", self.span.line, self.span.col, self.message)
    }
}
...
impl Error {
    pub fn new(span: Span, msg:&str) -> Self {
        Self{span, message:String::from(msg)}
    }
}
